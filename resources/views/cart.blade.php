                        ...
                        @foreach ($cartItems as $item)
                            <div class="flex items-center rounded-md border-2 hover:bg-gray-100 mx-1.5 my-5 px-6 py-5">
                                ...
                                <div class="w-32">
                                    ...
                                    @endphp
                                    <img class="h-24" src="{{ asset('storage/' . $firstImage) }}"
                                        alt="{{ $item['name'] }}">
                                </div>
                                <!-- Affichage des détails du produit -->
                                <div class="flex flex-col justify-between gap-2 ml-4 w-2/5">
                                    <span class="font-bold text-sm font-roboto text-slate-900">{{ $item['name'] }}</span>
                                    <span
                                        class="text-slate-50 w-14 font-readex font-medium text-xs text-center rounded-full bg-amber-500 py-0.5 px-1">{{ $item['brand'] }}</span>
                                    <span class="text-lg font-semibold text-rose-500 font-readex">{{ $item['price'] }}
                                        DH</span>
                                </div>
                                <!-- Zone pour modifier la quantité -->
                                <div class="flex justify-center w-2/5">
                                    <div class="flex items-center gap-1">
                                        <button type="button"
                                            class="size-10 leading-10 font-readex text-lg text-gray-600 transition hover:opacity-75">-</button>
                                        <input type="number"
                                            class="h-7 w-14 rounded outline-rose-500 border-slate-300 border-2 px-0.5 sm:text-base text-center font-xs font-readex quantity-input"
                                ...
                                <span id="total_{{ $item['product_id'] }}"
                                    class="text-center w-2/5 text-xl font-semibold text-rose-500 font-readex total-price">
                                    {{ isset($item['total']) ? $item['total'] : $item['price'] }} DH
                                </span>
                                <!-- Bouton pour supprimer le produit du panier -->
                                <div>
                                    <button
                                        class="inline-block p-3 text-gray-700 hover:text-rose-600 focus:relative delete-product-btn"
                ...
                <div id="summary" class="w-1/4 h-fit px-8 py-10 rounded-lg border-2 border-slate-300">
                    ...
                    <div class="flex justify-between items-center mt-10 mb-5">
                        <span id="cartItemCount"
                            class="font-medium text-sm text-slate-50 font-roboto rounded-full px-2 py-1 bg-green-500">
                    ...
                    <div class="flex justify-between items-center mt-3 mb-3">
                        ...
                        <span id="totalPrice" class="text-base font-bold text-slate-000 font-readex price">
                            0 DH
                        </span>
                    </div>
                    <div class="pb-4 pt-2">
